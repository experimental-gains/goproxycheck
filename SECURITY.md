...
### 2026-09-21 — toolchain and dependency CVEs (fixed in v0.1.9)

Versions v0.1.8 and earlier built against `go 1.24.4` and
...
- 26 reachable CVEs in the `go1.24.4` standard library (`crypto/tls`,
  ...
  through `goproxycheck`'s own use of `exec.Command` and
  `http.Client.Get` to talk to the Go module proxy — confirmed
  reachable with `govulncheck ./...`, not just present in `go.sum`.

Fixed in v0.1.9 by bumping to `go 1.26.8` and `golang.org/x/mod
v0.41.0`. `govulncheck ./...` reports no vulnerabilities as of that
release. If you're running v0.1.8 or earlier, upgrade.
